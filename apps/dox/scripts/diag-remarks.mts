...
const want = new Set([
  "RelativeUnit", "ZonedNowUnit", "UtcNowUnit", "UtcUnit",
  "UnixNowUnit", "TimeZoneNameStyle", "FormatUtcOptions",
  "FormatCalendarUnixOptions", "DstTransition",
]);
let found = 0;
for (const file of files) {
  ...
  for (const stmt of sf.statements) {
    ...
    const symName = stmt.name ? checker.getSymbolAtLocation(stmt.name) : undefined;
    ...
    for (const [label, sym] of candidates) {
      if (!sym) { console.log(`  ${label}: (no symbol)`); continue;
      }
