...
describe("intervalContainsUnix", () => {
  ...
  `(
    ...
    ({ intervalStart, intervalEnd, pointOrStart, pointEnd, expected }) => {
      expect(
        intervalContainsUnix(intervalStart, intervalEnd, pointOrStart, pointEnd),
      ).toBe(expected);
    },
  );
  ...
  `("returns false for non-numeric input: $intervalStart, $intervalEnd, $pointOrStart", ({ intervalStart, intervalEnd, pointOrStart }) => {
    expect(
      intervalContainsUnix(
        intervalStart as never,
        intervalEnd as never,
  ...
  `("returns false for non-numeric 4-arg input", ({ intervalStart, intervalEnd, innerStart, innerEnd }) => {
    expect(
      intervalContainsUnix(
        intervalStart as never,
        intervalEnd as never,
  ...
  `("accepts string inputs: $pointOrStart", ({ intervalStart, intervalEnd, pointOrStart }) => {
    expect(
      intervalContainsUnix(intervalStart, intervalEnd, pointOrStart, undefined),
    ).toBe(true);
  });
});
