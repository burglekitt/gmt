...
13 stories across five groups. **Each story ships something visible.** Group DOX-A produces
a real, deployed, searchable docs site; DOX-B through DOX–E are additive on top of a site that already
...
- **DOX-A2. Deploy** — GitHub Actions → GitHub Pages, static build on push to `main`,
...
- **DOX-A3. Reference generator** — The heart of the epic.
  ...
  **TypeScript compiler API** (not regex) and emits one MDX page per exported function
  ...
  retrieves from) and a **route manifest** (every URL generated, which DOX-C3 uses to reject
...
- **DOX-A4. Guides** — Hand-curated MDX under `src/content/docs/guides/`, ported from
...
## Story Group DOX-B — Live examples
...
- **DOX-B2. Auto-embed** — Extend DOX-A3's generator to mark up each `@example` so it renders as
  ...
  DOX-A3 with a real parser: the call/result split it already produces is exactly the
  seed data DOX-B2 needs.

## Story Group DOX-C — Ask Dox (the chatbot)
...
- **DOX-C1. Retrieval index** — Extend DOX-A3's `gmt-corpus.json` into retrieval chunks:
...
- **DOX-C2. Worker proxy** — `workers/dox-proxy` with `wrangler.toml`; key via
  ...
  this question, then `SKILL.md` vocabulary _before_ reference material, then GMT's core
...
- **DOX-C3. Chat panel** — A dismissible panel on the docs site, built from DOX-A5's tokens.
  ...
  href the model emits is checked against DOX-A3's route manifest, and anything not in it
...
## Story Group DOX-D — HUD identity
...
## Story Group DOX-E — Globe

- **DOX-E1. Landing-page hero** — A wireframe globe with meridians, as a `client:visible`
