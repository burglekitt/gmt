...
> Counts in this file **drift**: every `DOX-A3a` run re-derives them from source. The
> 504 / 1,860 / 42 figures below are a snapshot, not a guarantee.
>
...
- **The `@example` format is non-standard, but it is not the generator's risk — the
  ...
  across all 1,860 examples, **1,859 match that one shape exactly, zero contain a second
  `//`, and exactly one is multi-line** (`getDstTransitions`, whose array result
  continues on following `* //` lines). The parser is roughly fifteen lines and one edge
  case. See "The real generator risk" below for where DOX-A3a's attention actually
  belongs.
...
- **The exports map forbids per-function imports, and namespace imports drag the
  ...
  **namespace**-level import (`@northguild/gmt/plain`) pulls the entire polyfill — 2.98 MB
  ...
  module granularity, never namespace granularity** — and never per-function, which the
  exports map forbids outright.
...
- **GitHub Pages is not enabled on this repo** — `gh api repos/northguild/gmt/pages`
  returns 404. This is moot under the Cloudflare hosting decision (§2), but it is why
  that decision costs nothing: there is no existing Pages setup to migrate away from.
- **Octane is real but is off the critical path.** `octane` (MIT, by Dominic Gannaway /
  trueadm), with `@octanejs/three`, `@octanejs/drei`, and `@octanejs/markdown` — the
  adapters this epic would have depended on are still at `0.0.x`. It is a moving pre-1.0
  target with no benefit for a documentation site. **Nothing in any tier depends on it.**
  Revisit only as an optional island if it matures.
- **Counts drift, so derive them.** `context/roadmap/` is complete (all 54 stories Done
  through E7 at v1.14.0), so churn no longer comes from in-flight roadmap work — it comes
  from ordinary npm releases, which continue. DOX-A3a derives its counts from source and
  asserts them in CI rather than hardcoding a snapshot.
...
The `@example` parser is not the risk — signatures, options objects, and the absent tag
graph are. What will actually be hard in DOX-A3a:
