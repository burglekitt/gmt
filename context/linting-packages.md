...
| `@burglekitt/gmt-oxlint` | Oxlint ≥ 1 | JS plugin with custom rule modules (alpha) | `npm i -D @burglekitt/gmt-oxlint oxlint` |
| `@burglekitt/gmt-biome` | Biome ≥ 2 | GritQL `.grit` plugins in the `plugins` array | `npm i -D @burglekitt/gmt-biome @biomejs/biome` |
...
**How it works:** Each rule exports a `RuleModule` with a `meta` block and a `create(context)` function returning AST visitor methods. The plugin registers all rules under the `@burglekitt/gmt-oxlint` namespace. A `recommendedConfig` export bundles `jsPlugins` + `rules` for one-line setup.

**Rule structure:**
```ts
export const noNewDateRule: RuleModule = {
  meta: { type: "problem", docs: { description: "Disallow new Date(...)." }, schema: [] },
  create(context) {
    return {
      NewExpression(node) {
...
4. Add `"@burglekitt/gmt-oxlint/no-<pattern>": "error"` to `config/recommended.json`
