...
**One exception to "additive-only":** J0b moves two functions between published subpaths (`@northguild/gmt/plain/get` → `.../plain/calculate`). Root imports are unaffected, but deep-subpath importers break. Treat it as `minor` with an explicit changeset note naming both paths — not as a silent additive change. See [issues/J.md](issues/J.md).
